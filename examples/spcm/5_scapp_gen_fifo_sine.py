...
with spcm.Card(
    ...
    output_divider = 10
    time_range = (
        cp.arange(notify_samples_magnitude, dtype=cp.float32) / sample_rate_magnitude
    )
    data_raw_gpu = cp.empty(
        (len(channels), notify_samples_magnitude), dtype=cp_dtype, order="F"
    )
    frequency1 = 1000  # Hz
    frequency2 = 1002  # Hz
    ...
    try:
        ...
        for card_buffer in scapp_transfer:
            ...
            card_buffer[0, :] = (
                cp.sin(2 * cp.pi * ((time_range + phase) * frequency1))
                * 0.5
                * max_value
            )
            card_buffer[1, :] = (
                cp.sin(2 * cp.pi * ((time_range + phase) * frequency2))
                * 0.5
                * max_value
            )
