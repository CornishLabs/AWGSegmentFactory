...
prog = (
    ...
    .segment("left_up", mode="once")
        .tones("V").move(df=+2.0e6, time=200e-6, idxs=[0])

    # 2) move left trap right (H only, idx 0)
    .segment("left_right", mode="once")
        .tones("H").move(df=+1.0e6, time=200e-6, idxs=[0])

    # 3) ramp it down (amp of trap 0 on both planes; keep trap “present”)
    .segment("left_ramp_down", mode="once")
        .tones("H").ramp_amp_to(amps=[0.1], time=1e-3, idxs=[0], kind="exp", tau=0.2e-3)
        .tones("V").ramp_amp_to(amps=[0.1], time=1e-3, idxs=[0], kind="exp", tau=0.2e-3)

    # 4) undo: ramp up, move left, move down (all within one segment)
    .segment("left_undo", mode="once")
        .tones("H").ramp_amp_to(amps=[0.7], time=1e-3, idxs=[0], kind="exp", tau=0.2e-3)
        .tones("V").ramp_amp_to(amps=[0.7], time=1e-3, idxs=[0], kind="exp", tau=0.2e-3)
        .tones("H").move(df=-1.0e6, time=200e-6, idxs=[0])
        .tones("V").move(df=-2.0e6, time=200e-6, idxs=[0])
    ...
    .segment("right_up", mode="once")
        .tones("V").move(df=+2.0e6, time=200e-6, idxs=[1])

    .segment("right_right", mode="once")
        .tones("H").move(df=+1.0e6, time=200e-6, idxs=[1])

    .segment("right_ramp_down", mode="once")
        .tones("H").ramp_amp_to(amps=[0.1], time=1e-3, idxs=[1], kind="exp", tau=0.2e-3)
        .tones("V").ramp_amp_to(amps=[0.1], time=1e-3, idxs=[1], kind="exp", tau=0.2e-3)

    .segment("right_undo", mode="once")
        .tones("H").ramp_amp_to(amps=[0.7], time=1e-3, idxs=[1], kind="exp", tau=0.2e-3)
        .tones("V").ramp_amp_to(amps=[0.7], time=1e-3, idxs=[1], kind="exp", tau=0.2e-3)
        .tones("H").move(df=-1.0e6, time=200e-6, idxs=[1])
        .tones("V").move(df=-2.0e6, time=200e-6, idxs=[1])
