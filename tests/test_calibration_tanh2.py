...
class TestAODTanh2Calib(unittest.TestCase):
    def test_inverse_matches_forward_for_constant_coeffs(self) -> None:
        ...
        calib = AODTanh2Calib(
            ...
            amp_scale=1.0,
            min_g=1e-12,
            # Make v0 ~= 1.0 without being exactly 0 (required by validation).
            min_v0_sq=1e-30,
            y_eps=1e-6,
        )
    ...
    def test_used_in_sample_synthesis_pipeline(self) -> None:
        ...
        # Single tone, phase=pi/2 so sample 0 is exactly the amplitude.
        ...
        expected_amp = float(np.arctanh(np.sqrt(0.25)))
