...
class TestSequenceCompile(unittest.TestCase):
    ...
    def test_quantize_wrap_snapping_propagates_to_next_segment_when_continuous(
        self,
    ) -> None:
        fs = 1000.0
        f0 = 7.0
        ...
        seg1 = q_ir.segments[1]
        f_end = float(seg0.parts[-1].logical_channels["H"].end.freqs_hz[0])
        f_start = float(seg1.parts[0].logical_channels["H"].start.freqs_hz[0])
        self.assertAlmostEqual(f_start, f_end, places=12)
